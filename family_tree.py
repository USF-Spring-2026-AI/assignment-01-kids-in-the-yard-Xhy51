...
class PersonFactory:
    ...
    def _read_last_names(self):
        ...
        # the file has Decade, Rank, LastName columns - bucket by decade so
        # we can pick a historically appropriate last name for each person
        with open(path, newline="", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                decade = row.get("Decade") or row.get("decade")
                name = row.get("LastName") or row.get("last_name") or row.get("lastname")
                rank_str = row.get("Rank") or row.get("rank")
                if not decade or not name or not rank_str:
                    continue
                rank = int(rank_str)
    ...
    def compute_year_died(self, year_born):
        ...
        age = int(round(le)) + random.randint(-10, 10)
    ...
    def get_person(self, year_born, is_descendant, root_last_names, forced_last=None):
        ...
        if forced_last is not None:
            ...
            last = random.choice(list(root_last_names))
        else:
...
class FamilyTree:
    ...
    def build(self):
        ...
        root_last = self.factory.pick_last_name(1950)

        p1 = self.factory.get_person(1950, is_descendant=True,
                                     root_last_names=(root_last, root_last),
                                     forced_last=root_last)
    ...
    def _maybe_add_partner(self, person):
        ...
        yb = max(1950, min(2120, person.year_born + random.randint(-10, 10)))
    ...
    def _make_children(self, person):
        ...
        n_kids = random.randint(min_kids, max_kids)
        ...
        for _ in range(n_kids):
            yb = random.randint(start_year, end_year)
